...
public class UI {
    ...
    public void init(Telemetry telem, Gamepad _gamepad){
        gamepad = _gamepad;
        telemetry = telem;
    }
    ...
    public boolean floatInput(String label, FloatPtr value, float step){
        itemCount++;
    ...
    public boolean intInput(String label, IntPtr value, int step){
        itemCount++;
        addLine(label + ": " + value.value);
        if(selectedItem == itemCount) {
            if (right() && !prevRight) {
    ...
    public boolean checkbox(String label, BoolPtr value){
        itemCount++;
